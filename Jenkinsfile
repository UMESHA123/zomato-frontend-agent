...
//  Replace frontend-agent and zomato-frontend-agent when splitting
// ============================================================
pipeline {
    agent any
    ...
    environment {
        ...
        REPO_NAME          = 'zomato-frontend-agent'
    ...
    stages {
        ...
        // ==================== PARALLEL: LINT + TYPE CHECK + AUDIT ====================
        stage('Quality Checks') {
            parallel {
                ...
                stage('Type Check') {
                    steps {
                        script {
                            def hasTsc = sh(script: 'node -e "const p=require(\'./package.json\'); const d={...p.dependencies,...p.devDependencies}; process.exit(d.typescript ? 0 : 1)"', returnStatus: true) == 0
                            if (hasTsc) {
                                sh 'npx tsc --noEmit'
                            } else {
                                echo "TypeScript not in dependencies — skipping"
                            }
                        }
                    }
                }
                    ...
                    retry(5) {
                        ...
                        sh """
                            HTTP_CODE=\$(curl -s -o /dev/null -w '%{http_code}' \
                                --max-time 10 http://\${STAGING_SERVER}:\${STAGING_PORT}/)
                            if [ "\$HTTP_CODE" != "200" ]; then
                                echo "Staging smoke test returned \$HTTP_CODE"
                                exit 1
                            fi
